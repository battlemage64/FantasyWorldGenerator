...
class WorldMap:
    def __init__(self, seed=input("Enter a seed:\n>>> ")):
        random.seed(seed)

        takenboxes = []

        for i in range(random.randint(4, 7)):
            cont = cb.MapContinent(random.random()) # gen's continent with seed based on map's seed
            for tile in cont.maptiles:
                xoffset = tile[0].position[0]
                yoffset = tile[0].position[1]
                continent_polygon = canvas.create_polygon(tile[0].points, fill=BIOMECOLORS[tile[0].biome], outline='', tags=("bbox"))
        ...
        ##                    positiontaken = True
            ...
            for tile in cont.maptiles:
                if contyoffset < 200 or contyoffset > 800: # if necessary, replace desert with tundra
                    if tile[0].biome == "desert":
                        tile[0].biome = "tundra"
                xoffset = contxoffset + tile[0].position[0]
                yoffset = contyoffset + tile[0].position[1]
                continent_polygon = canvas.create_polygon(tile[0].points, fill=BIOMECOLORS[tile[0].biome], outline='', tags=("current"))
                canvas.move(continent_polygon, xoffset, yoffset)

                for lake in tile[1]:
                    if __name__ == '__main__':
                        lake_polygon = canvas.create_polygon(lake.points, fill='#0000FF', outline='')
                        canvas.move(lake_polygon, xoffset, yoffset)
                
            for river in cont.rivers: # ([points], width)
                riverpts = []
