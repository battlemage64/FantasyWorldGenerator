...
class MapTile:
    def __init__(self, draw=True, lake=False, parent=None, offset=20, seed=None, trend=random.randint(1, 8)):
        if not seed:
            random.seed()
        else:
            random.seed(seed)
        ...
        if not lake:
            ...
            point = (-1, -1)
            while not parent.cells[point[0]][point[1]]:
...
if __name__ == '__main__':
    tile = MapTile(False)
    grid_polygon = canvas.create_polygon(tile.outline_points, fill='', activefill='#00FF00')
    perturbed_polygon = canvas.create_polygon(tile.perturbed_points, fill='#00FF00', outline='#FF0000')
